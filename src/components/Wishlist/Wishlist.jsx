              ...
              {wishList.map((product, i) => (
                <div key={i} className="col-md-3 h-25 ">
                  <div className="product py-3 px-2 position-relative px-3 overflow-hidden ">
                    ...
                    {product.id !== likedItem ? (
                      <i onClick={() => removeProductFromWishList(product.id)} className="fa-solid fa-heart position-absolute top-0 text-danger end-0 p-1 "><i></i></i>
                    ) : (
                      ""
                    )}
                    <Link to={`/details/${product.id}`} className="text-decoration-none ">
                      <img src={product.imageCover} className="w-100 p-2"/>
